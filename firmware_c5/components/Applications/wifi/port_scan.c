...
#define UDP_RESULT_CLOSED     (-1)
#define UDP_RESULT_FILTERED   0
#define UDP_RESULT_OPEN       1
#define UDP_TIMEOUT_US        (PORT_SCAN_UDP_TIMEOUT_MS * 1000)
...
int port_scan_target_range(const char *target_ip,
  ...
  for (int port = start_port; port <= end_port; port++) {
    ...
    if (udp >= 0)
      add_result(target_ip, port, udp, banner, results, &count, max_results);
...
static bool check_tcp(const char *ip, int port, char *banner_out) {
  ...
  timeout.tv_sec = PORT_SCAN_CONNECT_TIMEOUT_S;
  timeout.tv_usec = 0;
  ...
  send(sock, "\r\n", 2, 0);
  int len = recv(sock, banner_out, PORT_SCAN_MAX_BANNER_LEN - 1, 0);
...
// Returns: -1 (Closed), 0 (Open|Filtered), 1 (Open+Data)
static int check_udp(const char *ip, int port, char *banner_out) {
  struct sockaddr_in dest_addr;
  dest_addr.sin_addr.s_addr = inet_addr(ip);
  dest_addr.sin_family = AF_INET;
  ...
  int len = recv(sock, banner_out, PORT_SCAN_MAX_BANNER_LEN - 1, 0);
  int result = UDP_RESULT_CLOSED;

  if (len > 0) {
    result = UDP_RESULT_OPEN;
    sanitize_banner(banner_out, len);
  } else {
    strcpy(banner_out, "(Without Banner)");
    if (errno == ECONNREFUSED) {
      result = UDP_RESULT_CLOSED;
    } else if (errno == EAGAIN || errno == EWOULDBLOCK) {
      result = UDP_RESULT_FILTERED;
    }
  }
  close(sock);
  return result;
...
static bool add_result(const char *ip,
  ...
  res->port = port;
  res->protocol = (scan_type == 0) ? PORT_SCAN_PROTO_TCP : PORT_SCAN_PROTO_UDP;
  strncpy(res->banner, banner, PORT_SCAN_MAX_BANNER_LEN);
  res->status =
      (scan_type == 0 || scan_type == 1) ? PORT_SCAN_STATUS_OPEN : PORT_SCAN_STATUS_OPEN_FILTERED;

  const char *p_str = (scan_type == 0) ? "TCP" : "UDP";
  const char *s_str = (res->status == PORT_SCAN_STATUS_OPEN) ? "OPEN" : "FILTERED";
