...
// TentacleOS: body factored out of esp_vfs_fat_sdcard_format_cfg so the mkfs
// format kind (FM_ANY vs FM_EXFAT) can be selected. See this component's README.
static esp_err_t format_card_fm(const char *base_path, sdmmc_card_t *card, esp_vfs_fat_mount_config_t *cfg, BYTE fmt_kind)
{
    ...
    const MKFS_PARM opt = {fmt_kind, (s_ctx[id]->mount_config.use_one_fat ? 1 : 2), 0, 0, alloc_unit_size};
    res = f_mkfs(drv, &opt, workbuf, workbuf_size);
    free(workbuf);
    if (res != FR_OK) {
        ret = ESP_FAIL;
...
{
    return format_card_fm(base_path, card, cfg, (BYTE)FM_ANY);
}

esp_err_t esp_vfs_fat_sdcard_format_exfat(const char *base_path, sdmmc_card_t *card)
{
    return format_card_fm(base_path, card, NULL, (BYTE)FM_EXFAT);
}
