...
esp_err_t storage_file_rename(const char *old_path, const char *new_path) {
  ...
  storage_resolve_path(old_path, old_full, sizeof(old_full));
  storage_resolve_path(new_path, new_full, sizeof(new_full));
...
esp_err_t storage_file_copy(const char *src, const char *dst) {
  ...
  char src_full[VFS_MAX_PATH], dst_full[VFS_MAX_PATH];
  storage_resolve_path(src, src_full, sizeof(src_full));
  storage_resolve_path(dst, dst_full, sizeof(dst_full));
...
esp_err_t storage_file_compare(const char *path1, const char *path2, bool *equal) {
  ...
  *equal = false;
  ...
  char full1[VFS_MAX_PATH], full2[VFS_MAX_PATH];
  storage_resolve_path(path1, full1, sizeof(full1));
  storage_resolve_path(path2, full2, sizeof(full2));
  ...
  uint8_t buf1[COMPARE_BUF_SIZE], buf2[COMPARE_BUF_SIZE];
  ssize_t r1, r2;
