...
static int alloc_slot(void)
{
    ...
    for (int i = 0; i < MT_PKT_HISTORY_MAX; i++) {
        if (s_table[i].rx_time_ms == 0) return i;
...
{
    if (relayer_id == 0) return;
    for (int i = 0; i < MT_NUM_RELAYERS; i++) {
        if (record->relayed_by[i] == relayer_id) return;
    }
    for (int i = 0; i < MT_NUM_RELAYERS; i++) {
...
void mt_pkt_history_init(void)
{
    ...
    ESP_LOGI(TAG, "Initialized - %d slots, %d relayers per entry",
             MT_PKT_HISTORY_MAX, MT_NUM_RELAYERS);
}
...
bool mt_pkt_history_check_add(uint32_t sender, uint32_t id,
                               ...
                               bool *out_upgraded)
{
    if (out_upgraded != NULL) *out_upgraded = false;
    ...
    if (hop_limit > highest_seen) {
        record->hop_limit_packed = (record->hop_limit_packed & ~HOP_HIGHEST_MASK) |
                                    (hop_limit & HOP_HIGHEST_MASK);
        if (out_upgraded != NULL) *out_upgraded = true;
    }
...
bool mt_pkt_history_was_relayer(uint8_t relayer_id, uint32_t sender, uint32_t id)
{
    ...
    for (int i = 0; i < MT_NUM_RELAYERS; i++) {
        if (s_table[idx].relayed_by[i] == relayer_id) return true;
    }
    return false;
}
...
int mt_pkt_history_count(void)
{
    int count = 0;
    for (int i = 0; i < MT_PKT_HISTORY_MAX; i++) {
        if (s_table[i].rx_time_ms != 0) count++;
    }
    return count;
}
