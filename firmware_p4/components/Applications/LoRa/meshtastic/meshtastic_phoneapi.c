...
static uint16_t build_queue_status(uint8_t *out, uint32_t rid, int32_t res, uint32_t pkt_id) {
  uint8_t qs[24];
  uint16_t ql = 0;
  ...
  while (i < len) {
    ...
    if (wire_type == 5) {
      if (i + 4 > len)
        break;
      if (field == 6) {
        return (uint32_t)buf[i] | ((uint32_t)buf[i + 1] << 8) | ((uint32_t)buf[i + 2] << 16) |
               ((uint32_t)buf[i + 3] << 24);
      }
      i += 4;
    } else if (wire_type == 0) {
...
static void enqueue_queue_status(uint32_t pkt_id, int32_t res) {
  ...
  ESP_LOGI(TAG,
           "QueueStatus enqueued: pkt_id=0x%08lX res=%ld flen=%u %s",
...
static uint16_t build_filemanifest_entry(uint8_t *out, uint32_t rid, uint8_t idx) {
  if (idx >= PA_NUM_FILE_ENTRIES)
    return 0;
  uint8_t fi[64];
  uint16_t fi_len = 0;
  size_t nl = strlen(PA_FILE_ENTRIES[idx].name);
  fi_len +=
      enc_field_bytes(&fi[fi_len], 1, (const uint8_t *)PA_FILE_ENTRIES[idx].name, (uint16_t)nl);
  ...
  while (i < pb_len) {
    ...
    if (wire_type == 2) {
      ...
      if (field == 1) {
        ...
        ESP_LOGI(TAG,
                 "ToRadio.packet %lu bytes id=0x%08lX -> mesh_send",
