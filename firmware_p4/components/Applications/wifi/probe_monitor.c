...
probe_monitor_record_t *probe_monitor_get_result_by_index(uint16_t index) {
  spi_header_t resp;
  if (spi_bridge_send_command(
          SPI_ID_SYSTEM_DATA, (uint8_t *)&index, 2, &resp, (uint8_t *)&s_cached_probe, sizeof(s_cached_probe), 1000) ==
      ESP_OK) {
...
bool probe_monitor_save_results_to_sd_card(void) {
  return (spi_bridge_send_command(SPI_ID_WIFI_PROBE_SAVE_SD, NULL, 0, NULL, NULL, 0, 5000) == ESP_OK);
}
