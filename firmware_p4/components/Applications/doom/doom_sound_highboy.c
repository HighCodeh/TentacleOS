...
static boolean I_HB_InitSound(boolean use_sfx_prefix) {
  ...
  if (!s_started) {
    xTaskCreatePinnedToCoreWithCaps(
        mixer_task, "doom_snd", 4096, NULL, 6, NULL, 0, MALLOC_CAP_SPIRAM);
    s_started = true;
  }
  return true;
}
...
static int I_HB_GetSfxLumpNum(sfxinfo_t *sfx) {
  char nm[16];
  if (s_use_prefix)
    snprintf(nm, sizeof(nm), "ds%s", sfx->name);
  else
    snprintf(nm, sizeof(nm), "%s", sfx->name);
...
static int I_HB_StartSound(sfxinfo_t *sfx, int channel, int vol, int sep) {
  ...
  uint32_t plen = (uint32_t)data[4] | ((uint32_t)data[5] << 8) | ((uint32_t)data[6] << 16) |
                  ((uint32_t)data[7] << 24);
  if (rate == 0)
    rate = OUT_RATE;
  if (plen + 8 > (uint32_t)lumplen)
    plen = (uint32_t)lumplen - 8;
...
static void I_HB_StopSound(int ch) {
  if (ch >= 0 && ch < NUM_CH)
    s_ch[ch].active = false;
}
static boolean I_HB_SoundIsPlaying(int ch) {
  return (ch >= 0 && ch < NUM_CH) && s_ch[ch].active;
}
...
static void I_HB_CacheSounds(sfxinfo_t *sounds, int num) {
  (void)sounds;
  (void)num;
}
...
static boolean M_Init(void) {
  return true;
}
static void M_Shutdown(void) {}
static void M_SetVol(int v) {
  (void)v;
}
static void M_Pause(void) {}
static void M_Resume(void) {}
static void *M_Register(void *d, int l) {
  (void)d;
  (void)l;
  return NULL;
}
static void M_Unregister(void *h) {
  (void)h;
}
static void M_Play(void *h, boolean loop) {
  (void)h;
  (void)loop;
}
static void M_Stop(void) {}
static boolean M_IsPlaying(void) {
  return false;
}
