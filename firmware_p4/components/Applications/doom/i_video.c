...
void cmap_to_fb(uint8_t *out, uint8_t *in, int in_pixels) {
  ...
  for (i = 0; i < in_pixels; i++) {
    ...
    if (s_Fb.bits_per_pixel == 16) {
      ...
      // Assuming RGBA8888
      pix = (c.r << s_Fb.red.offset) | (c.g << s_Fb.green.offset) | (c.b << s_Fb.blue.offset);
...
void I_InitGraphics(void) {
  ...
  printf(
      "I_InitGraphics: framebuffer: x_res: %d, y_res: %d, x_virtual: %d, y_virtual: %d, bpp: %d\n",
      s_Fb.xres,
      s_Fb.yres,
      s_Fb.xres_virtual,
      s_Fb.yres_virtual,
  ...
  printf("I_InitGraphics: framebuffer: RGBA: %d%d%d%d, red_off: %d, green_off: %d, blue_off: %d, "
         "transp_off: %d\n",
         s_Fb.red.length,
         s_Fb.green.length,
         s_Fb.blue.length,
...
void I_FinishUpdate(void) {
  ...
  x_offset = (((s_Fb.xres - (SCREENWIDTH * fb_scaling)) * s_Fb.bits_per_pixel / 8)) /
             2; // XXX: siglent FB hack: /4 instead of /2, since it seems to handle the resolution
                // in a funny way
  // x_offset     = 0;
