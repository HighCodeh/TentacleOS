...
static void chan_enable(struct minigb_apu_ctx *ctx, const uint_fast8_t i, const bool enable) {
  uint8_t val;

  ctx->chans[i].enabled = enable;
  val = (ctx->audio_mem[0xFF26 - AUDIO_ADDR_COMPENSATION] & 0x80) | (ctx->chans[3].enabled << 3) |
        (ctx->chans[2].enabled << 2) | (ctx->chans[1].enabled << 1) | (ctx->chans[0].enabled << 0);
...
static void update_square(struct minigb_apu_ctx *ctx, audio_sample_t *samples, const bool ch2) {
  ...
  for (uint_fast16_t i = 0; i < AUDIO_NSAMPLES; i += 2) {
    ...
    while (update_freq(c, &pos)) {
      ...
      c->val = (c->square.duty & (1 << c->square.duty_counter)) ? VOL_INIT_MAX / MAX_CHAN_VOLUME
...
static uint8_t
wave_sample(struct minigb_apu_ctx *ctx, const unsigned int pos, const unsigned int volume) {
  uint8_t sample;
...
static void update_wave(struct minigb_apu_ctx *ctx, audio_sample_t *samples) {
  ...
  for (uint_fast16_t i = 0; i < AUDIO_NSAMPLES; i += 2) {
    ...
    while (update_freq(c, &pos)) {
      c->val = (c->val + 1) & 31;
      sample += ((pos - prev_pos) / c->freq_inc) * ((audio_sample_t)c->wave.sample - 8) *
                (AUDIO_SAMPLE_MAX / 64);
      c->wave.sample = wave_sample(ctx, c->val, c->volume);
      prev_pos = pos;
    }

    sample += ((audio_sample_t)c->wave.sample - 8) * (audio_sample_t)(AUDIO_SAMPLE_MAX / 64);
    {
...
static void update_noise(struct minigb_apu_ctx *ctx, audio_sample_t *samples) {
  ...
  for (uint_fast16_t i = 0; i < AUDIO_NSAMPLES; i += 2) {
    ...
    while (update_freq(c, &pos)) {
      c->noise.lfsr_reg = (c->noise.lfsr_reg << 1) | (c->val >= VOL_INIT_MAX / MAX_CHAN_VOLUME);
 ...
 */
void minigb_apu_audio_callback(struct minigb_apu_ctx *ctx, audio_sample_t *stream) {
...
#if FREQ_INC_MULT == 16
        128, 1024, 512, 341, 256, 205, 171, 146
#elif FREQ_INC_MULT == 64
        512, 4096, 2048, 1365, 1024, 819, 683, 585
...
#error "LUT not calculated for this value of FREQ_INC_MULT"
  ...
  if (i == 0) {
    ...
    c->sweep.inc =
        c->sweep.rate ? ((128u * FREQ_INC_REF) / (c->sweep.rate * AUDIO_SAMPLE_RATE)) : 0;
    c->sweep.counter = FREQ_INC_REF;
  }
...
uint8_t minigb_apu_audio_read(struct minigb_apu_ctx *ctx, const uint16_t addr) {
  static const uint8_t ortab[] = {0x80, 0x3f, 0x00, 0xff, 0xbf, 0xff, 0x3f, 0x00, 0xff, 0xbf,
                                  0x7f, 0xff, 0x9f, 0xff, 0xbf, 0xff, 0xff, 0x00, 0x00, 0xbf,
                                  0x00, 0x00, 0x70, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                  0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

  return ctx->audio_mem[addr - AUDIO_ADDR_COMPENSATION] | ortab[addr - AUDIO_ADDR_COMPENSATION];
}
 ...
 */
void minigb_apu_audio_write(struct minigb_apu_ctx *ctx, const uint16_t addr, const uint8_t val) {
...
void minigb_apu_audio_init(struct minigb_apu_ctx *ctx) {
  ...
  {
    const uint8_t regs_init[] = {0x80, 0xBF, 0xF3, 0xFF, 0x3F, 0xFF, 0x3F, 0x00,
