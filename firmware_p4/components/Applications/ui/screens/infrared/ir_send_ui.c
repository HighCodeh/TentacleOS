...
static void update_scroll_bar(void) {
    if (s_scroll_bar == NULL || s_file_count <= 1) return;
    int32_t pos = s_track_y_start +
        (s_selected * (s_track_h - 20)) / (s_file_count - 1);
...
static void scan_ir_files(void) {
    ...
    while ((proto_ent = readdir(root)) != NULL && s_file_count < MAX_FILES) {
        if (proto_ent->d_name[0] == '.' || proto_ent->d_type != DT_DIR) continue;
        ...
        while ((file_ent = readdir(sub)) != NULL && s_file_count < MAX_FILES) {
            ...
            if (len < 4 || strcmp(file_ent->d_name + len - 3, ".ir") != 0) continue;

            snprintf(s_file_names[s_file_count], sizeof(s_file_names[0]),
                     "[%.30s] %.60s", proto_ent->d_name, file_ent->d_name);
            snprintf(s_file_paths[s_file_count], sizeof(s_file_paths[0]),
                     "%.128s/%.128s", proto_ent->d_name, file_ent->d_name);
            s_file_count++;
        }
        closedir(sub);
    }
...
static void build_list(void) {
    if (s_items_cont != NULL) lv_obj_clean(s_items_cont);
    scan_ir_files();
...
void ui_ir_send_open(void) {
    if (s_screen != NULL) { lv_obj_del(s_screen); s_screen = NULL; }
    ...
    track_pts[1].x = 0; track_pts[1].y = s_track_h;
    ...
    if (sb_dsc == NULL) sb_dsc = assets_get("/assets/icons/slide_bar_v.bin");
    s_scroll_bar = lv_image_create(s_screen);
    if (sb_dsc != NULL) lv_image_set_src(s_scroll_bar, sb_dsc);
