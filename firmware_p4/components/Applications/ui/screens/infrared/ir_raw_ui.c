...
#define FOOTER    "OK send   S save   BACK"
...
static void build_scope_card(void) {
  ...
  lv_obj_set_style_bg_color(dot, lv_color_hex(COL_OK), 0);
  lv_obj_set_style_bg_opa(dot, LV_OPA_COVER, 0);

  lv_obj_t *cap_lbl = lv_label_create(cap_grp);
  lv_label_set_text(cap_lbl, "CAPTURED");
  lv_obj_set_style_text_font(cap_lbl, &lv_font_montserrat_12, 0);
  lv_obj_set_style_text_color(cap_lbl, lv_color_hex(COL_OK), 0);
  ...
  lv_obj_set_style_line_opa(scope, LV_OPA_COVER, 0);
...
static void build_saved_row(void) {
  ...
  lv_label_set_text(tag, "Saved as");
  ...
  lv_label_set_text(name, SAVED_NAME);
...
static void nav_timer_cb(lv_timer_t *t) {
  ...
  if (ok && !s_ok_last) {
    notify(NOTIFY_INFO, "Sending RAW signal");
